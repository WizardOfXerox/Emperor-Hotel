...
Room capacity is intentionally simplified as display text only. The current database no longer stores `capacity_adults`, `capacity_children`, `adults`, or `children` columns because reservation forms do not collect guest-count splits.
...
Capacity note: the UI displays a simple "up to 5 people" room note, but the current schema does not store separate capacity or adult/child count columns.
