...
Payment references are generated by the PHP `Payment` model when a payment is saved. Staff should not type these manually. Manual payments use a `PAY-` prefix, while simulated transactions use a `SIM-` prefix. Cash reservations create a pending cashier reference during reservation creation; credit card, debit card, bank transfer, online payment, and other methods continue to the Payments page.

The PHP `Payment` model also prevents active `Pending` and `Confirmed` payments from exceeding the reservation total. `Failed` and `Refunded` records stay in the transaction log but do not reduce the payable balance.
