...
function processRoomImageUploads(?array $existingRoom = null): ?string
{
    ...
    // Issue #25 Fix: Clean up orphaned uploaded images when clearing
    if ($existingRoom && !empty($existingRoom['image_url']) && !empty($_POST['clear_custom_images'])) {
        $rawImg = trim((string)$existingRoom['image_url']);
        $oldPaths = [];
        if (str_starts_with($rawImg, '[')) {
            $decoded = json_decode($rawImg, true);
            if (is_array($decoded)) {
                $oldPaths = array_values(array_filter(array_map('trim', $decoded)));
            }
        }
        if (empty($oldPaths)) {
            $oldPaths = array_values(array_filter(array_map('trim', explode(',', $rawImg))));
        }
        foreach ($oldPaths as $oldPath) {
            // Only delete uploaded files (not default assets)
            if (str_contains($oldPath, '/uploads/')) {
                $fullPath = __DIR__ . '/../' . ltrim($oldPath, '../');
                if (is_file($fullPath)) {
                    @unlink($fullPath);
                }
            }
...
<!-- CREATE ROOM MODAL -->
<?php require_once __DIR__ . '/../includes/create_room_modal.php'; ?>
